...
All plotting functions for the PrimeTrade sentiment analysis project.
Every function returns a matplotlib Figure and saves to FIGURES_DIR.
"""
import sys
...
def plot_sentiment_distribution(fear_greed: pd.DataFrame) -> plt.Figure:
    ...
    fig.suptitle("Fear & Greed Index — Sentiment Distribution", fontsize=15, y=1.02, color=ACCENT_COLOR)
...
def plot_pnl_by_sentiment(pnl_df: pd.DataFrame) -> plt.Figure:
    ...
    # Mean PnL
    ax = axes[0]
    bars = ax.bar(pnl_df["classification"], pnl_df["mean_pnl"],
    ...
    for b in bars:
        h = b.get_height()
        ax.text(b.get_x() + b.get_width() / 2, h + (0.5 if h >= 0 else -1.5),
                f"${h:.1f}", ha="center", fontsize=8)

    # Win Rate
    ax2 = axes[1]
    ax2.bar(pnl_df["classification"], pnl_df["win_rate"] * 100,
            color=colors, edgecolor="none")
    ...
    # Trade Count
    ax3 = axes[2]
    ax3.bar(pnl_df["classification"], pnl_df["trade_count"],
...
def plot_leverage_by_sentiment(lev_df: pd.DataFrame) -> plt.Figure:
    ...
    bars = ax.bar(lev_df["classification"], lev_df["mean_leverage"],
...
def plot_long_short_ratio(ls_df: pd.DataFrame) -> plt.Figure:
    ...
    w = 0.35
    b1 = ax.bar(x - w / 2, ls_df["longs"], width=w, label="Long", color=POSITIVE_COLOR, alpha=0.85)
    b2 = ax.bar(x + w / 2, ls_df["shorts"], width=w, label="Short", color=NEGATIVE_COLOR, alpha=0.85)
    ax.set_xticks(x)
    ...
    ax2 = ax.twinx()
    ax2.plot(x, ls_df["long_ratio"] * 100, color=ACCENT_COLOR, marker="o",
             lw=2, label="Long Ratio %", zorder=5)
...
def plot_lag_correlation(lag_df: pd.DataFrame) -> plt.Figure:
    _style()
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(lag_df["lag_days"], lag_df["pearson_r"],
            marker="o", color=ACCENT_COLOR, lw=2, label="Pearson r")
    ax.plot(lag_df["lag_days"], lag_df["spearman_r"],
            marker="s", color="#ff9f43", lw=2, linestyle="--", label="Spearman r")
    ax.fill_between(lag_df["lag_days"], lag_df["pearson_r"], 0,
...
# ─── 8. Contrarian Strategy Equity Curve ─────────────────────────────────────
...
def plot_contrarian_strategy(backtest: dict) -> plt.Figure:
    ...
    fig.suptitle("Contrarian Strategy: Buy Extreme Fear / Sell Extreme Greed",
                 fontsize=14, color=ACCENT_COLOR)

    ax = axes[0]
    ax.plot(daily["date"], daily["strategy_equity"], lw=2,
            color=POSITIVE_COLOR, label=f"Contrarian  α={summary['alpha']:.1f}")
    ax.plot(daily["date"], daily["benchmark_equity"], lw=1.5,
            color="#aaa", linestyle="--", label="Benchmark (random)")
    ax.fill_between(daily["date"], daily["strategy_equity"],
                    daily["benchmark_equity"],
...
def plot_sentiment_timeline(fear_greed: pd.DataFrame, df: pd.DataFrame) -> plt.Figure:
    ...
    fig.suptitle("Market Sentiment & Trader Performance Over Time",
                 fontsize=14, color=ACCENT_COLOR)
    ...
    ax = axes[0]
    ax.fill_between(fg["date"], fg["value"],
                    where=fg["value"] <= 40, color="#d62728", alpha=0.3, label="Fear zone")
    ax.fill_between(fg["date"], fg["value"],
                    where=fg["value"] >= 60, color="#2ca02c", alpha=0.3, label="Greed zone")
    ...
    ax2 = axes[1]
    ax2.bar(daily_pnl["date"], daily_pnl["closedPnL"],
            color=[POSITIVE_COLOR if v > 0 else NEGATIVE_COLOR for v in daily_pnl["closedPnL"]],
            width=1, alpha=0.5)
    ax2.plot(daily_pnl["date"], daily_pnl["rolling_7d"],
             color=ACCENT_COLOR, lw=2, label="7-day rolling mean PnL")
...
def plot_transition_matrix(matrix: pd.DataFrame) -> plt.Figure:
    ...
    ax.set_title("Sentiment Regime Transition Probability (%)",
...
# ─── 11. PnL Distribution Violin ─────────────────────────────────────────────

def plot_pnl_violin(df: pd.DataFrame) -> plt.Figure:
    _style()
    closes = df[df["is_close"] & df["closedPnL"].notna()].copy()
    # Clip extreme outliers for better visualisation
    q01 = closes["closedPnL"].quantile(0.01)
    q99 = closes["closedPnL"].quantile(0.99)
