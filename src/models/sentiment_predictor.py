"""Random Forest classifier predicting trade profitability from sentiment context."""
...
class SentimentTradePredictor:
    """Binary classifier: profitable vs unprofitable trade given sentiment features."""
    ...
    def _encode_features(self, df: pd.DataFrame, fit: bool = False) -> pd.DataFrame:
        ...
        daily_sent = closes.groupby("date")["sentiment_num"].first().reset_index()
        daily_sent["rolling_sentiment_7d"] = (
            daily_sent["sentiment_num"].rolling(7, min_periods=1).mean()
        )
        closes = closes.merge(daily_sent[["date", "rolling_sentiment_7d"]], on="date", how="left")
        ...
        return closes[self.FEATURES + ["is_profitable"]].dropna()
    ...
    def evaluate(self) -> dict:
        ...
        cv_scores = cross_val_score(self.model, all_X, all_y, cv=5, scoring="roc_auc", n_jobs=-1)

        return {
            "classification_report": classification_report(self._y_test, y_pred, output_dict=True),
            "roc_auc":          round(auc, 4),
    ...
    def feature_importance_df(self) -> pd.DataFrame:
        ...
        return (
            pd.DataFrame({"feature": self.FEATURES, "importance": self.model.feature_importances_})
            .sort_values("importance", ascending=False)
        )


def plot_model_results(results: dict, figures_dir: Path) -> None:
    """Generate model evaluation charts (ROC, feature importance, confusion matrix)."""
    ...
    ax.plot(results["roc_curve"]["fpr"], results["roc_curve"]["tpr"],
            color=accent, lw=2, label=f"AUC = {results['roc_auc']:.3f}")
    ...
    ax2.set_facecolor(bg)
        ...
        pd.DataFrame(results["feature_importances"].items(), columns=["feature", "importance"])
